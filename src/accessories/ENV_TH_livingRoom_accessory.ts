...
console.log(location+" "+ "Connecting to MQTT broker...");
var client = mqtt.connect('mqtt://192.168.1.200');
client.on('connect', function () {
  client.subscribe('sensors/'+location, function (err: any) {
    if (err) {
      console.log(location+" "+ "failed to subscribe to MQTT broker")
    }
    else {
      console.log(location+" "+ "Sensor subscribed to MQTT broker")
    }
  })
})
...
client.on('message', function(topic: any, message: any) {
  ...
  console.log('MQTT realtime update...');
  console.log(location+ ' temperature: %s C, humidity: %s \%'

...
const outletIP = '192.168.1.179';

let tpOutlet = new TPLinkSmartPlug(outletIP );

// here's a fake hardware device that we'll expose to HomeKit
const FAKE_OUTLET = {
  powerOn: false,
  setPowerOn: (on: CharacteristicValue) => {
    console.log("Turning the" + location +" outlet %s!...", on ? "on" : "off");
    if (on) {
...
tpOutlet.turnOn();
      ...
      console.log(location +" outlet is now on.");
    } else {
...
tpOutlet.turnOff();
      ...
      console.log(location +" outlet is now off.");
    }
  },
  identify: function () {
    console.log("Identify the " +location +" outlet.");
  }
};
...
const outletUUID = uuid.generate('hap-nodejs:accessories:Outlet'+location);
...
outlet
  ...
  .setCharacteristic(Characteristic.Manufacturer, "uTPLink")
...
outlet
  .addService(Service.Outlet, location+" Outlet") // services exposed to the user should have "names" like "Fake Light" for us

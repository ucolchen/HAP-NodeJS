...
const location='livRoom';
const bulbIP = '192.168.1.101';

class LightControllerClass {

  name: CharacteristicValue = location+" Light"; //name of accessory
  ...
  manufacturer: CharacteristicValue = "u-tpl-LB120"; //manufacturer (optional)
  ...
  setPower(status: CharacteristicValue) { //set power of accessory
    ...
    this.power = status;
    if(status) exec('tplight on ' + bulbIP);
    if(!(status)) exec('tplight off ' + bulbIP);
    }
  ...
  setBrightness(brightness: CharacteristicValue) { //set brightness
	...
	exec('tplight bright ' + bulbIP + ' ' + brightness.toString());
  }
  ...
  setSaturation(saturation: CharacteristicValue) { //set saturation
    if(this.outputLogs) console.log("Setting '%s' saturation to %s", this.name, saturation);
    this.saturation = saturation;
	exec('tplight hsb ' + bulbIP + ' ' + this.hue.toString() + ' ' + saturation.toString());
  }
  ...
  setHue(hue: CharacteristicValue) { //set hue
    if(this.outputLogs) console.log("Setting '%s' hue to %s", this.name, hue);
    this.hue = hue;
	exec('tplight hsb ' + bulbIP + ' ' + hue.toString() + ' ' + this.saturation.toString());
  }
